...
A(1,[1 2 3]) = [m-3*m/dy/2+2*h_o*sinh(y(end)) 2*m/dy -m/dy/2];
A(end,[end-2 end-1 end]) = [m/dy/2 -2*m/dy m+3*m/dy/2-2*h_o*sinh(y(end))];
for i = 2:(length(y)-1)
    A(i,i-1) = -dt*((2+dy)*h_e*m-2*dy*h_o*sinh(y(i)))/(2*h_e*dy^2);
    A(i,i) = 1+dt*(1+2*m/dy^2-2*h_o/h_e*cosh(y(i)));
    A(i,i+1) = dt*((dy-2)*h_e*m-2*dy*h_o*sinh(y(i)))/(2*h_e*dy^2);
